...
Next.js 14 기반의 대한민국 공휴일을 쉽고 빠르게 확인할 수 있는 웹 서비스입니다. 공공데이터포털의 한국천문연구원 특일정보 API를 활용하여 정확한 공휴일 정보를 제공합니다.

## ✨ 주요 기능

- 📅 **연도별 공휴일 조회** - 현재 년도 기준 앞 1년, 뒤 1년 탭으로 간편하게 탐색
- 📆 **연간 한눈에 보기** - 12개월을 4열 그리드로 한 화면에 표시
- 🏷️ **공휴일 분류 표시** - 국경일과 대체공휴일을 색상으로 구분
- 🎉 **연휴 정보 표시** - 주말을 포함한 연휴 기간 자동 계산 및 표시 (3일 이상)
- 📱 **반응형 디자인** - 모바일, 태블릿, 데스크톱 모든 기기 지원
- 🎯 **오늘 날짜 하이라이트** - 현재 날짜를 시각적으로 강조
- ⚡ **빠른 로딩** - 24시간 캐싱으로 최적화된 성능
- 🎨 **모던한 다크 테마** - Pretendard 폰트와 white/gray 컬러톤의 세련된 UI

## 📋 사전 요구사항

- **Node.js** 18.0 이상
- **npm** 9.0 이상 또는 **yarn** 1.22 이상
- **공공데이터포털 API 키** (무료 발급 가능)
...
2. 환경 변수 `PUBLIC_DATA_API_KEY` 설정
3. 자동으로 배포됩니다
...
│   ├── page.tsx              # 메인 페이지 (공휴일 조회 UI)
│   └── page.module.css       # 페이지 스타일 (반응형 디자인)
├── public/                   # 정적 파일
...
- **다크 테마**: 배경 #0a0a0f, 텍스트 white/gray 톤
- **색상 구분**: 
  - 국경일: 파란색 계열 (#60a5fa)
  - 대체공휴일: 빨간색 계열 (#f87171)
- **4열 그리드**: 데스크톱에서 월별 섹션을 4열로 표시

## 🐛 트러블슈팅

### API 키 오류

**문제**: "API 키가 설정되지 않았습니다" 오류 발생

**해결 방법**:
1. `.env.local` 파일이 프로젝트 루트에 있는지 확인
2. 환경 변수 이름이 `PUBLIC_DATA_API_KEY`인지 확인
3. API 키가 올바르게 입력되었는지 확인 (공백 없이)
...
**해결 방법**:
1. 공공데이터포털에서 API 키가 활성화되어 있는지 확인
2. API 키의 인코딩/디코딩 버전을 사용하는지 확인
3. 네트워크 연결 상태 확인
4. API 일일 호출 제한 확인 (10,000건)
